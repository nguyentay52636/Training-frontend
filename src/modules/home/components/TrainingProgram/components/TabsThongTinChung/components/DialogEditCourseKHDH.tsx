...
interface DialogEditCourseKHDHProps {
  course?: CourseType;
  knowledgeId?: string;
...
export default function DialogEditCourseKHDH({
  course,
  knowledgeId,
  onCourseUpdated,
  mode = 'create'
}: DialogEditCourseKHDHProps) {
  ...
  useEffect(() => {
    if (course && mode === 'edit') {
      setMaHP(course.maHP || '');
      setTenHP(course.tenHP || '');
      setSoTinChi(course.soTinChi || 0);
      setSoTietLyThuyet(course.soTietLyThuyet || 0);
      setSoTietThucHanh(course.soTietThucHanh || 0);
      setSoTietThucTap(course.soTietThucTap || 0);
      setLoaiHocPhan(course.loaiHocPhan || '0');
      setTongSoTiet(course.tongSoTiet || 0);
      setHeSoHocPhan(course.heSoHocPhan || 1);
    }
  }, [course, mode]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'edit' && course?.idHocPhan) {
        // Update existing course
        await updateCourse(course.idHocPhan, courseData);
      ...
      toast.error(mode === 'edit' ? "Có lỗi xảy ra khi cập nhật học phần" : "Có lỗi xảy ra khi thêm học phần");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className={`cursor-pointer ${mode === 'edit'
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-[1.8rem]">
            {mode === 'edit' ? 'Chỉnh sửa học phần' : 'Thêm học phần mới'}
          </DialogTitle>
