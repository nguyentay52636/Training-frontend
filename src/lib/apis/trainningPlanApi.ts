...
export const  createCourseSpecialized =  async ({maHP, tenHP, soTinChi, soTietLyThuyet, soTietThucHanh, soTietThucTap, loaiHocPhan, tongSoTiet, heSoHocPhan, hocKy}: CourseType) => {  
const course = { 
    maHP : maHP,
    tenHP : tenHP,
    soTinChi : soTinChi,
...
export const  createSpecialized = async ({tenChuyenNganh, idHocPhan, hocKyThucHien}: SpecializedType) => { 
const specialized =  { 
...
export const deleteCourseBySpecialized = async (specializedId: number, courseId: number) => { 
    try {
        const { data } = await baseApi.delete<SpecializedType>(`/kehoachdayhoc/${specializedId}/hocphan/${courseId}`);
        return data;
    } catch (error) {
        throw error;
    }
